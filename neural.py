...
def loadImgs(tPath, vPath=None, testPath=None):
    ...
    testImg = loadImgsAux(testPath, translateASCIIvalue)
    return tImg, vImg, testImg
...
value = getInput("-1 to test the accuracy and cost of the trained network, other key for training: ")
if (value == '-1'):
    n = len(training_data)
    v_len = len(validation_data)
    t_len = len(test_data)
    accuracy = net.accuracy(training_data, convert=True)
    print "Accuracy on training data: {:.2%}".format(accuracy*1.0/n)
    accuracy = net.accuracy(validation_data)
    print "Accuracy on validation data: {:.2%}".format(accuracy*1.0/v_len)
    accuracy = net.accuracy(test_data)
    print "Accuracy on test data: {:.2%}".format(accuracy*1.0/t_len)

while (True):
    #	print "TRAINGING NEURAL NET?"
    #	value = getInput("-1 for exit, other key for trainging: ")
    if (value == '-1'):
        break
    print "TRAINING NEURAL NET"
    print "###################"
    net.SGD(training_data, epochs, batch, eta, evaluation_data=test_data, lmbda=lmbda, monitor_training_accuracy=True,
            monitor_evaluation_accuracy=(vPath != None))
